...
class QueuedActionRepositoryImpl extends QueuedActionRepository {
  ...
  Future<Result<bool>> executeQueuedAction(QueuedActionEntity queue) async {
    try {
      // If offline, do not attempt remote execution — keep queued actions persisted.
      if (!await pingService.isConnected) {
        return Result.failure(
          error:
              'Offline mode: queued actions execution is disabled (no network).',
        );
      }
  ...
  Future<Result<Null>> _functionSelector(QueuedActionEntity queue) async {
    try {
      ...
      if (queue.repository == 'StorageRepositoryImpl') {
        if (storageRemoteDatasource == null) {
          return Result.failure(
            error: 'Storage remote datasource not configured',
          );
        }

        if (queue.method == 'uploadUserPhoto') {
          final imgPath = queue.param;
        ...
        if (queue.method == 'uploadProductImage') {
          ...
          final res = await storageRemoteDatasource!.uploadProductImage(
            imgPath,
          );
          if (res.isFailure) return Result.failure(error: res.error!);
