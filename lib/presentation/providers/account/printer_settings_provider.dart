...
class PrinterSettingsProvider extends ChangeNotifier {
  ...
  Set<PrinterConnectionType> get selectedTypes =>
      Set.unmodifiable(_selectedTypes);
  ...
  int get selectedPrinterIndex {
    ...
    final selectedId = printerService.getDeviceId(
  ...
  Future<void> getAndSelectPrinter() async {
    ...
    final selectedDeviceId = sharedPreferences.getString(
      Constants.selectedDeviceIdKey,
    );
  ...
  Future<void> onSelectPrinter(PrinterDevice printer) async {
    ...
    sharedPreferences.setString(
      Constants.selectedConnectionTypeKey,
  ...
  bool _hasSamePrinters(List<PrinterDevice> printers) {
    ...
    for (int i = 0; i < printers.length; i++) {
      if (printerService.getDeviceId(_printers[i]) !=
          printerService.getDeviceId(printers[i])) {
        return false;
      }
    }
